...
double precision :: prob(10)
integer :: num_of_intervals , j  ,count = 0             ! by just reallocate the array
...
do j = 1, num_of_intervals
    ...
    if (j == 1000*count+1) then  ! the prob is use for make the electron cloud 
        prob(count+1) = si(j)    ! here prob store the value at the some points
        count = count + 1        ! that have max in range we can sum for all value 
    endif                        ! but i choose this way and it works fine
...
close(2)
    ...
    ! Making the electron cloud by calling the subroutine orbit 
...
contains  ! func is the wave equations
        ...
        subroutine orbit(probability)
            double precision , dimension(10) , intent(in):: probability
            integer :: i,k,points
            double precision :: t,r,x1,y1 
            logical :: f2 

            inquire(file="orbit.dat",exist=f2) 
            if(f2) then 
                open(3,file="orbit.dat",status="replace")
            else 
                open(3,file="orbit.dat",status="new",action="write")
            endif

            do k = 1,5
                points = probability(k)*100
                do i = 1, 1000
                    if (points < i) then ! here we generate the random no. in circle like 
                        call random_number(t)  ! the random position of electron and as the 
                        call random_number(r)  ! probability of find the electron increase the 
                        r = r*real(k)          ! the number of the points are also increasing 
                        x1 = r*cos(t*2*pi) + 1 ! and we get what we wanted and repeat this 
                        y1 = r*sin(2*pi*t)     ! process for both the protons 
                        write(3,*)x1,y1
                        x1 = r*cos(t*2*pi) - 1
                        y1 = r*sin(2*pi*t) 
                        write(3,*)x1,y1
